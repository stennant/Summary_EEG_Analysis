...
def plot_average_seizures(data, output_path):
    ...
    het_syn21 = df.loc[df['Treatment'] == 'HET - SYN21', 'Number of seizures']
    het_syn20 = df.loc[df['Treatment'] == 'HET - SYN20', 'Number of seizures']
    ...
    total_time_wake = [wt_pbs_mean, het_pbs_mean, het_syn21_mean, het_syn20_mean]
    average_sd = [wt_pbs_sd, het_pbs_sd, het_syn21_sd, het_syn20_sd]
    bins = np.arange(4)

    #color = ['OrangeRed', 'PeachPuff', 'SandyBrown', 'dimgrey']
    color = ['black', 'dimgrey', 'dodgerblue', 'LightSkyBlue']
    ...
    plt.ylabel('Average number of seizures', fontsize=12, labelpad=10)
...
def plot_seizure_duration(data, output_path):
    ...
    het_syn21 = df.loc[df['Treatment'] == 'HET - SYN21', 'Avg. seizure duration']
    het_syn20 = df.loc[df['Treatment'] == 'HET - SYN20', 'Avg. seizure duration']
    wt_pbs = df.loc[df['Treatment'] == 'WT - PBS', 'Avg. seizure duration']
    het_pbs = df.loc[df['Treatment'] == 'HET - PBS', 'Avg. seizure duration']
    ...
    ax = percent_histogram.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
    ax.bar(bins, total_time_wake, color = color, alpha=0.8)
    #ax.bar(3, het_syn20_mean, color = 'PeachPuff', hatch = '/')
    ax.plot(np.random.uniform(low=-0.2, high=0.2, size=(len(het_syn21))), np.array(het_syn21), color = "black", marker = 's', linestyle='None', markersize = 2)
    ax.plot(np.random.uniform(low=0.8, high=1.2, size=(len(het_syn20))), np.array(het_syn20), color = "black", marker = 's', linestyle='None', markersize = 2)
    ax.plot(np.random.uniform(low=1.8, high=2.2, size=(len(het_pbs))), np.array(het_pbs), color = "black", marker = 's', linestyle='None', markersize = 2)
    ax.plot(np.random.uniform(low=2.8, high=3.2, size=(len(wt_pbs))), np.array(wt_pbs), color = "black", marker = 's', linestyle='None', markersize = 2)
    plt.errorbar(bins, total_time_wake, average_sd, fmt='none', ls='', marker='o', capsize=5, capthick=1, ecolor='black')
    #plt.errorbar(3, het_syn20_mean, het_syn20_sd, fmt='none', ls='', marker='o', capsize=5, capthick=1, ecolor='black')
    plt.ylabel('Average seizure duration (seconds)', fontsize=12, labelpad=10)
    plt.locator_params(axis='x', nbins=6)
    ax.set_xticklabels(['', 'WT - PBS', 'HET - PBS', 'HET - SYN21', 'HET - SYN20'])
    plt.xticks(rotation=45)
    ax.spines['top'].set_visible(False)
...
def main():
    ...
    overall_data_path = '/Volumes/Sarah/SYNGAPE8/SyngapE8_Analysis.csv'
